...
const CONFIG = {
  /* 예약 프로그램별 정원 — 같은 타임을 공유하되 정원은 따로 관리한다.
     ...
     컬러링은 11타임 × 6명 = 66명이 상한이다. 수채화지 100장(소서노 30·문학산성 30·
     갯벌 30·수봉폭포 10)을 준비했으므로 재료가 아니라 좌석 수가 상한이 된다.
     바인더는 11타임 × 5명 = 55명이지만 체험용 종이바인더가 50개라 50에서 막는다. */
  PROGRAMS: [
    { key: '컬러링', capacity: 6, dayLimit: 66 },
    { key: '바인더', capacity: 5, dayLimit: 50 },
  ],

  /* 예약이 쌓이는 시트(탭) 이름.
     반드시 이 행사 전용 탭을 지정하세요. 빈 문자열로 두면 폼 연결 시트나 첫 번째 탭을
     자동으로 찾는데, 지난 행사 응답 시트가 잡히면 그때 예약이 이번 행사의 마감으로 잡힙니다.
     이 이름의 탭이 없으면 setupEvent() 가 헤더까지 만들어 줍니다. */
  SHEET_NAME: '예약_0919',
